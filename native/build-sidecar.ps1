...
Write-Host "=== arr-mcp sidecar build ===" -ForegroundColor Cyan
...
try {
    ...
    Remove-Item -Recurse -Force "$Root\build\arr-mcp-backend" -ErrorAction SilentlyContinue
    Remove-Item -Force "$Root\dist\arr-mcp-backend.exe" -ErrorAction SilentlyContinue

    Write-Host "-> Running PyInstaller..." -ForegroundColor Yellow
    uv run pyinstaller arr-mcp-backend.spec --clean --noconfirm
